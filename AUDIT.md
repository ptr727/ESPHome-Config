...
This asserts the live repository settings against the hub's `settings.json`, the fleet label set
against the hub's `labels.json`, and each of the `main` and `develop` rulesets against the hub's
`main.json` and `operational/develop.json`, exiting non-zero on drift. It also asserts the
deployment environments the registry entry declares, and this repo declares none, so nothing is
asserted there and the live list is only read. A missing ruleset or a divergent payload is a **defect**. The result
