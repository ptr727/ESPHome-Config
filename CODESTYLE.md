...
**This repository's enforced gate is the fleet rule above, and its local task reaches further.** Validation runs through the hub's reusable `validate-task.yml`, which spell-checks `README.md` and `HISTORY.md` and exposes no input to widen that, so those two files are what CI enforces. The `Lint: Spelling` VS Code task covers `**/README.md`, [`DEVICES.md`][devices], and [`HISTORY.md`][history], which is a local convenience rather than a second gate: a typo in a nested README reaches CI unflagged and surfaces only to whoever runs that task or edits the file with the cspell editor extension. This repository ran the wider list in a cspell step of its own until that step was replaced with the hub task on 2026-08-18, and the note here claimed for four weeks afterward that CI still carried it. The templates, the easystart project, and its Python subtree each ship a README a reader lands on directly, so the task covers all of them, and `DEVICES.md` is the reader-facing device inventory split out of the root README (see [OPERATIONS.md][operations-documenting-a-device]), so the task covers it as the README it came from was covered by the fleet rule. It is deliberately **not** all `**/*.md`: this repository carries many Markdown files full of technical terms, and gating every one of them would mean endlessly padding [`cspell.json`][cspell] just to keep CI green. Broad, live spell-checking across any file is the cspell editor extension's job, so typos still surface to whoever is editing. Markdown *linting* stays repo-wide `**/*.md`, since it does not choke on technical terms. Where this note and the workflows disagree, the workflows are authoritative for this repository.
...
- **Match ESPHome's own naming** in an external component, meaning `snake_case` members and methods and a trailing underscore on private members, so the component reads like the framework it plugs into.
