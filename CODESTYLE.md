...
**This repository's CI spelling gate is the fleet default, and its local one reaches further.** Validation runs through the hub's reusable `validate-task.yml`, which spell-checks `README.md` and `HISTORY.md` and takes no input to widen that, so CI covers those two. The `Lint: Spelling` VS Code task covers `**/README.md`, [`DEVICES.md`][devices], and [`HISTORY.md`][history] instead. The two lists are deliberately different and only the CI one is enforced, so a typo in a nested README reaches CI unflagged and surfaces to whoever runs the task or edits the file with the cspell editor extension. This repository carried the wider list in its own CI step until [#100][pr-100] replaced that step with the hub task on 2026-08-18. The templates, the easystart project, and its Python subtree each ship a README a reader lands on directly, so the task covers all of them, and `DEVICES.md` is the reader-facing device inventory split out of the root README (see [OPERATIONS.md][operations-documenting-a-device]), so the task covers it as the README it came from was covered. It is deliberately **not** all `**/*.md`: this repository carries many Markdown files full of technical terms, and gating every one of them would mean endlessly padding [`cspell.json`][cspell] just to keep CI green. Broad, live spell-checking across any file is the cspell editor extension's job, so typos still surface to whoever is editing. Markdown *linting* stays repo-wide `**/*.md`, since it does not choke on technical terms. Where this note and the workflows disagree, the workflows are authoritative for this repository.
