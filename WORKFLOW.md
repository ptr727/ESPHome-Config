...
- **Shells.** Every bash surface, a multi-line `run:` block and every committed `.sh` script alike, starts `set -Eeuo pipefail`. This repository diverges: thirteen of its fourteen workflow steps carry `set -euo pipefail`, as does its one committed script, `easystart/tools/pull-apk.sh`. That is a gap in the code rather than in the rule.
