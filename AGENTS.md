...
**ESPHome-Config** is the live `/config` directory of a running ESPHome instance, so the tree is deployed state rather than a checkout that ships later. It holds the per-device configurations for one home automation setup, the reusable device and utility templates behind them, and the Micro-Air EasyStart reverse-engineering project.

This file is the entry point every coding agent reads first, and it holds only three things: the bootstrap that says where the canonical rules live and which procedure to follow for the state this repository is actually in, the rules for managing context and delegation, which apply to every task, and a map of where every other rule lives. The rule text itself is in [`GOVERNANCE.md`](./GOVERNANCE.md), one section per topic. Code style lives in [`CODESTYLE.md`](./CODESTYLE.md), the CI/CD workflow contract in [`WORKFLOW.md`](./WORKFLOW.md), and this repository's own audit procedure in [`AUDIT.md`](./AUDIT.md).

**Read [`OPERATIONS.md`](./OPERATIONS.md) before editing any YAML under this tree.** It is the ESPHome authority: reaching the CLI inside the container, validating and flashing a change, the template and vendor-firmware patterns, and the traps that have cost time before. The two human-facing docs are split by audience, [`README.md`](./README.md) being the reusable template catalog for anyone adopting one and [`DEVICES.md`](./DEVICES.md) the maintainer's own deployed fleet.

Treat this file and `GOVERNANCE.md` as authoritative for cross-cutting rules, and do not restate their rules elsewhere. A rule specific to this repository lives in whichever local doc owns its subject, meaning ESPHome operations and repository tooling in `OPERATIONS.md`, code and documentation style in `CODESTYLE.md`, and the workflow contract in `WORKFLOW.md`, never in [`.github/copilot-instructions.md`](./.github/copilot-instructions.md), which targets GitHub Copilot and VS Code specifically while this file and `GOVERNANCE.md` are the agent-agnostic ones every coding agent is directed to read.
