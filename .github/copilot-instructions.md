...
Do not duplicate language-specific rules here. **Project-specific conventions and behavioral contracts belong in the local doc that owns the subject, meaning [OPERATIONS.md](../OPERATIONS.md) for ESPHome operations and repository tooling, not here.** This repository's `GOVERNANCE.md` is carried fleet law and byte-locked, so a rule specific to this repository cannot live there. This file is intentionally limited to the inline commit/PR-title summary, the guidance for reviewing carried fleet content, and the GitHub Copilot Review Runbook. Non-Copilot agents (Claude Code, Codex, Cursor, ...) are not directed to this file and don't read it by default, so any rule a reviewer must honor has to live in `GOVERNANCE.md` or the local doc that owns it, routed to from `AGENTS.md`, to be provider-independent.
...
gh api repos/ptr727/ESPHome-Config/pulls/<N>/reviews --jq \
  '.[] | select([(.body // "") | split("\n")[] | select(. != "")][0] // ""
...
{
  repository(owner: "ptr727", name: "ESPHome-Config") {
...
gh api repos/ptr727/ESPHome-Config/issues/<N>/comments --jq \
...
`scripts/pr_review.py status <N> --repo ptr727/ESPHome-Config` reports this as `shapes=UNRECOGNIZED`, lists each shape under a marker naming the remedy, and exits `43`. `wait` carries the same code, so a wait cannot end on a clean zero over output nothing read. The vetted inventory lives in that script and is small on purpose: measured over 332 Copilot review bodies on this repository, with fenced blocks dropped and text reduced to ASCII, the whole corpus is seven headings, six `<summary>` texts and three metadata labels, and every body carries at least one of them. A body carrying none is itself the unrecognized shape, which is what catches a rewrite that changes everything at once, the refusal wording drifting among it.
...
gh api graphql -f query='
{ repository(owner:"ptr727",name:"ESPHome-Config"){ pullRequest(number:<N>){
...
gh api --paginate 'repos/ptr727/ESPHome-Config/issues/<N>/timeline?per_page=100' \
