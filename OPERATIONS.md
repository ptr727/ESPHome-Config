...
- **A unit appeared to go silent with no host reading the port, and that reading is withdrawn.** Every read in that session opened the tty, which the bullet above resets the board. The observation is confounded by the act of observing it. Keeping `uart_debug` at `WARN` is still worth doing, since a raw NMEA tap costs console bandwidth for nothing when nobody is reading.
