...
- **Time sync and a position fix have very different signal requirements.** Decoding the navigation message from one satellite carries date and time, and ESPHome's `gps` time platform needs both before it syncs. A position fix needs four satellites. A unit in a poor location therefore holds time indefinitely and never reports a coordinate.
