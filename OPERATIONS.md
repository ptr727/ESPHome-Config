...
- **Markdown links are reference-style in every file this repository owns, including this one.** The fleet rule names four agent-instruction files that keep inline links instead, [`AGENTS.md`][agents], [`GOVERNANCE.md`][governance], this one, and `.github/copilot-instructions.md`, on the grounds that they are read one section at a time so a definition at the foot of the file is never reached. This file carries reference-style links throughout, which is a divergence from that rule and a conversion to make deliberately rather than incidentally. Definitions live at the bottom of the file, grouped under `<!-- Repo -->` and `<!-- External -->` and alphabetized within each group. A reference name encodes what it points at, so `analog-max17048-link`, never `analog-en-products-link` after a URL path segment. Removing a link also removes its definition, since an orphan fails the lint.
