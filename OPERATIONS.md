...
- **Markdown links are reference-style everywhere except the four agent-instruction files** ([`AGENTS.md`][agents], [`GOVERNANCE.md`][governance], this file, and `.github/copilot-instructions.md`), which keep inline links because they are read one section at a time rather than end to end. Definitions live at the bottom of the file, grouped under `<!-- Repo -->` and `<!-- External -->` and alphabetized within each group. A reference name encodes what it points at, so `analog-max17048-link`, never `analog-en-products-link` after a URL path segment. Removing a link also removes its definition, since an orphan fails the lint.
