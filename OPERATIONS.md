...
- **Never mount the live checkout into a third-party lint container.** `secrets.yml` holds the live secrets and is git-ignored. [`secrets._yaml`][secrets-example] is the tracked placeholder. Build a temporary snapshot from tracked and intended untracked files, then mount that snapshot read-only. The Git exclusion rules keep `secrets.yml` out without maintaining a second exclusion list.
