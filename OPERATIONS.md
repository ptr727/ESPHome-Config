...
- **The board bus is an `i2c:` list entry, and a mapping there loses the bus in silence.** `merge_config` keeps only the consuming config's value when one side is a mapping and the other a list. Before this change the template held a mapping, so a consumer's list entry dropped the GPIO17 and GPIO18 bus with nothing reporting it. Two list entries concatenate instead, which is why [Waveshare ESP32-S3-ETH Camera][waveshare-camera] takes that form. A device on the merged bus then needs an explicit `i2c_id`, and says so with `Too many candidates found`. Every board template in the tree uses the list form and the id `board_i2c`.
...
- **The hardware is cleared, and indoor sky view was the whole problem.** Taken outdoors on battery the board reaches 8 satellites and a valid position fix, verified under other firmware. That clears the module, the antenna, the wiring, the direction, the baud and the protocol configuration. Indoors this overlay saw one satellite at most and never a valid RMC or ZDA, so its time source stayed unsynced. A satellite in GSV is not a decoded one, and only a decoded navigation message carries the date.
...
- **A unit appeared to go silent with no host reading the port, and that reading is withdrawn.** Every read in that session opened the tty, which the bullet above shows resets the board. The observation is confounded by the act of observing it. Keeping `uart_debug` at `WARN` is still worth doing, since a raw NMEA tap costs console bandwidth for nothing when nobody is reading.
