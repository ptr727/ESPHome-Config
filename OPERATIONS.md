...
What verifying a change here costs, and which half of this repository's contract CI structurally cannot reach. [`GOVERNANCE.md`][governance] "Verification Discipline" points every agent at this heading for that list, and the sections named below hold the commands.

- **Config and compile validation is the gate, and it runs in the `esphome` container**, never against a host `esphome` binary. "Container and CLI" has the invocation and "Config and Compile Validation" has what to run against which file. Run it before pushing a template or device change.
- **The doc gates are the fleet set and they run against the diff**, so run them from this worktree against the branch base rather than trusting a clean whole-tree pass. "Repository Tooling Hazards" holds the container invocations and the mounting rule that goes with them.
- **CI compiles one example device per published template, and that is the whole of what it proves.** A green run means the template builds against the current ESPHome release. It does not mean the device boots, that its pins are right for the board in hand, that a BLE or ethernet peripheral enumerates, or that the config resolves against the live `secrets.yaml`, since CI generates a dummy one.
- **Nothing in CI touches hardware, and nothing in CI flashes.** Every claim about a device working is a claim about a physical flash and a log capture, per "Flashing and sdkconfig" and "Logs and the API Connection Cap". A change to a board template, a strapping pin, a vendor-firmware conversion, or a component knob is unverified until it has run on the board it targets.
