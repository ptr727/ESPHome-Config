...
When wiring up a substitution that maps to an ESPHome component field with a constrained value set, meaning an enum or an allowed-values list, verify the valid values from the component source first. Grep the **running container's** copy, which is the version that actually validates and compiles:
...
- **Read the vendor's value before wiring a substitution into a vendor block.** The including side wins the scalar merge. A template default therefore overrides the vendor's value, even when it equals ESPHome's own default. Apollo sets `api: reboot_timeout: 0s` on purpose. A `15min` template default would reboot every PLT-1B that Prevent Sleep holds awake whenever Home Assistant is unreachable. Default the substitution to the vendor's value, as [Apollo PLT-1B][apollo-plt-1b] does, unless the point is to change it.
...
- **Config validators need type hints**, `def validate_x(config: ConfigType) -> ConfigType:`, with `ConfigType` imported from `esphome.types`. Older validators in the same file often lack them, and new code is still expected to have them.
...
- **Inline HTML is limited to `<details>` and `<summary>`.** Those two are allowed because a collapsible has no markdown equivalent. Every other element still fails `MD033`, and that includes a `<code>` nested inside a `<summary>`, so use a markdown code span there instead.
