...
- **The board bus is an `i2c:` list entry, and a mapping there loses the bus in silence.** `merge_config` keeps only the consuming config's value when one side is a mapping and the other a list. A device composing this template and declaring its own `i2c:` list entry drops the GPIO17 and GPIO18 bus. Nothing reports it. Two list entries merge instead, which is why [Waveshare ESP32-S3-ETH Camera][waveshare-camera] takes that form. A device on the merged bus then needs an explicit `i2c_id`, and says so with `Too many candidates found`. Every board template in the tree uses the list form and the id `board_i2c`.
...
- **Time sync and a position fix have very different signal requirements.** Decoding the navigation message from one satellite carries date and time, and ESPHome's `gps` time platform needs both before it syncs. A position fix needs four satellites.
- **This overlay reached neither time nor a fix on the bench, and the cause is not isolated.** It saw a time of day in GGA, never a date, and never more than one satellite, so the time source stayed unsynced. The same board did sync time under other firmware given an overnight run. That proves the receive path, meaning wiring, direction, baud, protocol and firmware. It does not clear the antenna: one demodulated satellite carries time, where ranging needs four at adequate signal. Placement and antenna are both still candidates.
- **Opening the `USB_SERIAL_JTAG` port can reset the MCU on this board, so a read destroys a run in progress.** A GNSS acquisition has to be one held-open capture. Reconnecting to check on it is what ends it, and this board persists no position, so there is nothing to recover afterwards.
- **A unit logging raw NMEA over `USB_SERIAL_JTAG` went silent with no host reading the port.** It ran and streamed whenever a reader was attached. It went unreachable within about a minute of the last one detaching, recovering only on an esptool reset. Dropping `uart_debug` to `WARN` cleared it, and the unit then ran unattended for 50 minutes. The mechanism is unconfirmed, so treat the remedy as what was observed to work rather than as a diagnosis.
- **The 0db range on an S3 is about 950mV, measured rather than taken from the datasheet.** A cell reading 4.18V to 4.19V on a meter gave raw 3657 of 4095 and a calibrated 868mV at the pin. Back-solving full scale from the raw count and the meter gives 955mV to 958mV. The often quoted 1100mV is the classic ESP32's reference and overstates a reading by about 15%.
- **That 15% applies to a raw count read against the hardware scale.** A framework that normalizes a calibrated reading onto a fixed 1100mV scale is a separate case, and its constant is correct there. Check which scale a count is on before applying either number.
- **Redo that measurement with a raw sensor beside the calibrated one.** Both need `allow_other_uses: true` on the pin. Two `adc` platforms on one pin otherwise fail with `Pin 1 is used in multiple places`. The nominal 4.9 ratio read 1.5% to 1.8% high against the meter, inside resistor tolerance, so the template keeps it.
...
- **A bench unit needs both reboot timeouts disabled.** `api.yaml` and [`wifi.yaml`][wifi-template] each default to a 15 minute `reboot_timeout`. A unit on a desk with no Home Assistant hits both, restarting part way through a cold GNSS acquisition. Override `api_reboot_timeout` and `wifi:` `reboot_timeout` to `0s`.
...
- **The spell-check gate CI enforces covers `README.md` and `HISTORY.md`, and nothing else.** That is the fleet rule, applied by the hub's reusable `validate-task.yml`, which exposes no input to widen it. The `Lint: Spelling` task covers `**/README.md` plus [`DEVICES.md`][devices] and `HISTORY.md`, so it reads wider than CI does and gates nothing. A typo in a nested README passes CI, and only that task or the cspell editor extension will show it.
